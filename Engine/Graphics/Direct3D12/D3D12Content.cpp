...
namespace spoon::graphics::d3d12::content {
	namespace {
		...
		struct {
			utl::vector<spoon::content::lod_offset>    lod_offsets;
			...
			create_pso(id::id_type material_id, D3D12_PRIMITIVE_TOPOLOGY primitive_topology, u32 elements_type)
		{
			...
			for (u32 i{ 0 }; i < shader_type::count; ++i)
			{
				if (flags & (1 << i))
				{
					spoon::content::compiled_shader_ptr shader{ spoon::content::get_shader(material.shader_ids()[shader_index]) };
					assert(shader);
	...
	namespace render_item {
		...
		//
		id::id_type
			add(id::id_type entity_id, id::id_type geometry_content_id,
				u32 material_count, const id::id_type *const material_ids)
		{
			...
			spoon::content::get_submesh_gpu_ids(geometry_content_id, material_count, gpu_ids);
			...
			get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
		{
			...
			spoon::content::get_lod_offsets(frame_cache.geometry_ids.data(), frame_cache.thresholds.data(), count, frame_cache.lod_offsets);
			...
			for (u32 i{ 0 }; i < count; ++i)
			{
				...
				const spoon::content::lod_offset& lod_offset{ frame_cache.lod_offsets[i] };
