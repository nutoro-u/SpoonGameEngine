...
#pragma intrinsic(_BitScanForward)
			...
			{ // Lock materials
				...
				for (u32 i{ 0 }; i < shader_type::count; ++i)
				{
					if (flags & (1 << i))
					{
						...
						spoon::content::compiled_shader_ptr shader{ spoon::content::get_shader(material.shader_ids()[shader_index], key) };
						assert(shader);
	...
	namespace render_item {
			...
			get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
		{
			...
			spoon::content::get_lod_offsets(frame_cache.geometry_ids.data(), info.thresholds, count, frame_cache.lod_offsets);
